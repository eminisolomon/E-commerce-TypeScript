...
const booleanValidator = (
  ...
  if (options?.optional) {
    ...
    validation.exists().withMessage(`${messageName} must be provided`).bail().notEmpty().withMessage(`${messageName} must be provided`).bail();
  }
    ...
    .custom(value => {
      if (!isEmpty(value)) {
        if (!options?.allow.some(v => v === value)) {
          throw new Error(`${messageName} must be a boolean: [${options?.allow.join(',')}]`);
        }
        return true;
      }
      return false;
...
const optionalTextField = (field: string, messageName: string, options: { min: number; max: number; nullable: boolean }) => {
  return body(field)
    .optional({
